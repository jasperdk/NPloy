...
namespace NPloy.Console.UnitTests.Commands
{
    ...
    public class UninstallPackageCommandTests
    {
        ...
        {
            // Arrange
            _nPloyConfiguration.Setup(f => f.FileExists(It.Is<string>(s => s.EndsWith(@"\NPloy.Samples.WindowsService.1.0.0.0\App_Install\Uninstall.ps1"))))
                            .Returns(true);
            ...
            _powershellRunner.Verify(
                p =>
                p.RunPowershellScript(@".\App_Install\Uninstall.ps1", null, It.Is<string>(s => s.EndsWith(@"\NPloy.Samples.WindowsService.1.0.0.0"))),
                Times.Once());
