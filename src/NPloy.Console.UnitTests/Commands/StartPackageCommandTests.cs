...
namespace NPloy.Console.UnitTests.Commands
{
    ...
    public class StartPackageCommandTests
    {
        ...
        {
            // Arrange
            _nPloyConfiguration.Setup(f => f.FileExists(@"\NPloy.Samples.WindowsService.1.0.0.0\App_Install\Start.ps1"))
                            .Returns(true);
            ...
            _powershellRunner.Verify(
                p =>
                p.RunPowershellScript(@".\App_Install\Start.ps1", @"\NPloy.Samples.WindowsService.1.0.0.0"),
                Times.Once());
